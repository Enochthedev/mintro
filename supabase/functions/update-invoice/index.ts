...
serve(async (req)=>{
  ...
  try {
    ...
    const { invoice_id, client, amount, status, due_date, invoice_date, service_type, notes, tags, items, transaction_ids, // Cost breakdown fields (for manual override)
    actual_materials_cost, actual_labor_cost, actual_overhead_cost, cost_override_reason } = await req.json();
    if (!invoice_id) {
      return new Response(JSON.stringify({
    ...
    const { data: currentInvoice } = await supabaseClient.from("invoices").select("source, quickbooks_invoice_id, amount, total_actual_cost, original_qb_amount, edited_after_sync").eq("id", invoice_id).eq("user_id", user.id).single();
    ...
    if (Object.keys(updates).length > 0) {
      const { data, error: updateError } = await supabaseClient.from("invoices").update(updates).eq("id", invoice_id).eq("user_id", user.id).select().single();
      if (updateError) {
        throw updateError;
      }
      if (!data) {
      ...
      const { data, error: fetchError } = await supabaseClient.from("invoices").select().eq("id", invoice_id).eq("user_id", user.id).single();
      if (fetchError) {
        throw fetchError;
      }
      if (!data) {
    ...
    if (items !== undefined) {
      ...
      await supabaseClient.from("invoice_items").delete().eq("invoice_id", invoice_id);
      ...
      if (items && Array.isArray(items) && items.length > 0) {
        ...
        const { data: insertedItems, error: itemsError } = await supabaseClient.from("invoice_items").insert(itemsToInsert).select(); // Get back the inserted items with their actual DB values
        if (itemsError) {
    ...
    if (transaction_ids !== undefined) {
      ...
      const { data: existingLinks } = await supabaseClient.from("transaction_job_allocations").select("id").eq("job_id", invoice_id);
      if (existingLinks && existingLinks.length > 0) {
        await supabaseClient.from("transaction_job_allocations").delete().eq("job_id", invoice_id);
        transactionsUnlinked = existingLinks.length;
      }
      // Link new transactions (each 100%)
      if (transaction_ids && transaction_ids.length > 0) {
        const { data: transactions } = await supabaseClient.from("transactions").select("id, amount, name").in("id", transaction_ids).eq("user_id", user.id);
        if (transactions) {
          ...
          const { error: linkError } = await supabaseClient.from("transaction_job_allocations").insert(allocations);
          if (!linkError) {
    ...
    if (items !== undefined || transaction_ids !== undefined) {
      ...
      const { data: allAllocations } = await supabaseClient.from("transaction_job_allocations").select("allocation_amount").eq("job_id", invoice_id);
      const transactionCosts = allAllocations?.reduce((sum, alloc)=>sum + Math.abs(Number(alloc.allocation_amount) || 0), 0) || 0;
      // Get line item costs (including overrides)
      const { data: lineItems } = await supabaseClient.from("invoice_items").select("category, qty, unit_price, is_override, override_cost").eq("invoice_id", invoice_id);
      const lineItemCosts = lineItems?.reduce((sum, item)=>{
        if (item.is_override) {
      ...
      const { data: finalInvoice } = await supabaseClient.from("invoices").update({
