...
serve(async (req)=>{
  ...
  try {
    ...
    if (!rule_type || !match_value) {
      return new Response(JSON.stringify({
        error: "rule_type and match_value are required"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: transactions, error: txError } = await supabaseClient.from("transactions").select("id, name, merchant_name, amount, date").eq("user_id", user.id).order("date", {
      ascending: false
    ...
    for (const tx of transactions){
      ...
      switch(rule_type){
        case "vendor_exact":
          isMatch = vendorName === matchValueLower || tx.name?.toLowerCase() === matchValueLower;
          break;
        case "vendor_contains":
          isMatch = vendorName.includes(matchValueLower) || tx.name?.toLowerCase().includes(matchValueLower);
          break;
        case "description_contains":
          isMatch = tx.name?.toLowerCase().includes(matchValueLower);
          break;
        case "amount_range":
          isMatch = (!min_amount || amount >= parseFloat(min_amount)) && (!max_amount || amount <= parseFloat(max_amount));
          break;
      }
      if (isMatch) {
