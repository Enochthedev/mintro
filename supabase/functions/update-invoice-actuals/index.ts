...
serve(async (req)=>{
  ...
  try {
    ...
    const { invoice_id, actual_materials_cost, actual_labor_cost, actual_overhead_cost, cost_override_reason } = await req.json();
    if (!invoice_id) {
      return new Response(JSON.stringify({
    ...
    const hasChanges = invoice.actual_materials_cost !== newMaterialsCost || invoice.actual_labor_cost !== newLaborCost || invoice.actual_overhead_cost !== newOverheadCost;
    if (hasChanges) {
      // Create override history record
      await supabaseClient.from("invoice_cost_overrides").insert({
        invoice_id,
        user_id: user.id,
    ...
    // Update invoice
    const { data: updated, error: updateError } = await supabaseClient.from("invoices").update({
