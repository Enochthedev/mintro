...
serve(async (req)=>{
  ...
  try {
    ...
    if (!blueprint_usages || blueprint_usages.length === 0) {
      return new Response(JSON.stringify({
        error: isBatch ? "blueprint_usages array cannot be empty" : "blueprint_id is required"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: blueprints, error: blueprintsError } = await supabaseClient.from("cost_blueprints").select(`
        *,
        blueprint_inventory_items (
          *,
          inventory_items (*)
        )
      `).eq("user_id", user.id).in("id", blueprintIds);
    if (blueprintsError) {
    ...
    if (!blueprints || blueprints.length !== blueprintIds.length) {
      ...
      return new Response(JSON.stringify({
        error: isBatch ? "One or more blueprints not found" : "Blueprint not found",
    ...
    if (deduct_inventory) {
      ...
      for (const [itemId, totalRequired] of inventoryAggregation.entries()){
        const { data: inventoryItem } = await supabaseClient.from("inventory_items").select("*").eq("id", itemId).single();
        if (inventoryItem) {
    ...
    const { data: createdUsages, error: usagesError } = await supabaseClient.from("blueprint_usage").insert(usagesToCreate).select();
    if (usagesError) {
    ...
    if (deduct_inventory) {
      for(let i = 0; i < blueprint_usages.length; i++){
        ...
        if (blueprint?.blueprint_inventory_items?.length > 0) {
          for (const item of blueprint.blueprint_inventory_items){
            ...
            const { data: currentItem } = await supabaseClient.from("inventory_items").select("current_quantity, minimum_quantity, name, unit_cost").eq("id", inventoryItemId).single();
            if (currentItem) {
              const newQty = (currentItem.current_quantity || 0) - quantityUsed;
              const { error: updateError } = await supabaseClient.from("inventory_items").update({
      ...
      if (inventoryTransactions.length > 0) {
        const { error: transactionsError } = await supabaseClient.from("inventory_transactions").insert(inventoryTransactions);
    ...
    const totalActualCost = createdUsages.reduce((sum, usage)=>sum + (usage.actual_materials_cost || 0) + (usage.actual_labor_cost || 0) + (usage.actual_overhead_cost || 0), 0);
    const totalActualRevenue = createdUsages.reduce((sum, usage)=>sum + (usage.actual_sale_price || 0), 0);
    ...
    if (isBatch) {
      return new Response(JSON.stringify({
        ...
        summary: {
          ...
          profit_margin: totalActualRevenue > 0 ? parseFloat((totalActualProfit / totalActualRevenue * 100).toFixed(2)) : 0
        },
