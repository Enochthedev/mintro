...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile } = await supabaseClient.from("business_profiles").select("subscription_tier").eq("id", user.id).single();
    ...
    const { data: syncCount } = await supabaseClient.from("plaid_sync_logs").select("id", {
      count: "exact",
      head: true
    ...
    const { data: plaidItems } = await supabaseClient.from("plaid_items").select("id").eq("user_id", user.id).eq("status", "active");
