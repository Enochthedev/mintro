...
serve(async (req)=>{
  ...
  try {
    ...
    // Get transactions
    const { data: transactions, error: txError } = await supabaseClient.from("transactions").select("*").in("id", transaction_ids).eq("user_id", user.id);
    if (txError || !transactions) {
      throw txError || new Error("Transactions not found");
    }
    // Get active blueprints
    const { data: blueprints, error: bpError } = await supabaseClient.from("cost_blueprints").select("*").eq("user_id", user.id).eq("is_active", true);
...
function checkBlueprintMatch(transaction, blueprint) {
  let confidence = 0;
  ...
  // Materials merchants
  if (merchantLower.includes('home depot') || merchantLower.includes('lowes') || merchantLower.includes('supply') || merchantLower.includes('lumber')) {
    confidence += 0.3;
    expense_type = 'materials';
    reasons.push('materials merchant');
  }
  // Labor/contractor
  if (merchantLower.includes('contractor') || merchantLower.includes('labor') || merchantLower.includes('payroll') || merchantLower.includes('wage')) {
    confidence += 0.4;
    expense_type = 'labor';
    reasons.push('labor keyword');
  }
  // Subscription services
  if (blueprint.blueprint_type === 'subscription' && transaction.amount > 0 && Math.abs(transaction.amount - blueprint.estimated_materials_cost) < 5) {
    confidence += 0.6;
    expense_type = 'overhead';
    reasons.push('matches subscription amount');
  }
  // Amount-based matching
  if (blueprint.estimated_materials_cost && Math.abs(transaction.amount) >= blueprint.estimated_materials_cost * 0.8 && Math.abs(transaction.amount) <= blueprint.estimated_materials_cost * 1.2) {
    confidence += 0.2;
