...
serve(async (req)=>{
  ...
  try {
    ...
    if (categorizations.length > 100) {
      return new Response(JSON.stringify({
        error: "Maximum 100 categorizations per request"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    for (const item of categorizations){
      if (!item.transaction_id || !item.category_id) {
        return new Response(JSON.stringify({
          error: "Each item must have transaction_id and category_id"
        }), {
          status: 400,
          headers: {
            ...corsHeaders,
    ...
    const { data: transactions, error: txError } = await supabaseClient.from("transactions").select("id").eq("user_id", user.id).in("id", transactionIds);
    if (txError) throw txError;
    const validTransactionIds = new Set(transactions?.map((t)=>t.id) || []);
    // Verify categories belong to user
    const { data: categories, error: catError } = await supabaseClient.from("expense_categories").select("id").eq("user_id", user.id).in("id", categoryIds);
    if (catError) throw catError;
    const validCategoryIds = new Set(categories?.map((c)=>c.id) || []);
    // Filter to valid categorizations
    const validCategorizations = categorizations.filter((c)=>validTransactionIds.has(c.transaction_id) && validCategoryIds.has(c.category_id));
    if (validCategorizations.length === 0) {
      return new Response(JSON.stringify({
        error: "No valid transaction/category pairs found"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: existingCategorizations } = await supabaseClient.from("transaction_categorizations").select("transaction_id").in("transaction_id", transactionIds);
    ...
    if (toInsert.length > 0) {
      ...
      const { data: inserted, error: insertError } = await supabaseClient.from("transaction_categorizations").insert(insertData).select();
      if (insertError) {
    ...
    for (const item of toUpdate){
      const { error: updateError } = await supabaseClient.from("transaction_categorizations").update({
    ...
    const allCategoryIds = [
      ...new Set(validCategorizations.map((c)=>c.category_id))
    ];
    const { data: categoryNames } = await supabaseClient.from("expense_categories").select("id, name").in("id", allCategoryIds);
    const categoryNameMap = new Map(categoryNames?.map((c)=>[
        c.id,
        c.name
      ]) || []);
