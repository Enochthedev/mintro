 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    const { data: allAllocations, error: allocError } = await supabaseClient.from("transaction_job_allocations").select("job_id, allocation_amount").in("job_id", invoiceIds);
    if (allocError) {
    ...
    const invoiceCalculations = invoices.map((inv)=>{
      ...
      const overrideCost = hasOverride && inv.total_actual_cost !== null ? parseFloat(inv.total_actual_cost || 0) : null;
    ...
    const serviceTypeAnalysis = Array.from(byServiceType.entries()).map(([type, data])=>({
        ...
        average_margin: data.total_revenue > 0 ? parseFloat((data.total_profit / data.total_revenue * 100).toFixed(2)) : 0,
    ...
    // ============================================
    const lowMarginJobs = invoiceCalculations.filter((job)=>job.margin < min_margin).sort((a, b)=>a.margin - b.margin).slice(0, 10).map((job)=>({
    ...
    const highMarginJobs = invoiceCalculations.sort((a, b)=>b.margin - a.margin).slice(0, 10).map((job)=>({
        invoice_id: job.invoice_id,
        invoice_number: job.invoice_number,
        client: job.client,
        service_type: job.service_type,
    ...
    const averageMargin = allMargins.length > 0 ? allMargins.reduce((sum, m)=>sum + m, 0) / allMargins.length : 0;
    ...
    return new Response(JSON.stringify({
      ...
      summary: {
        ...
        jobs_below_threshold_percent: invoices.length > 0 ? parseFloat((jobsBelowThreshold / invoices.length * 100).toFixed(2)) : 0
      },
      data_quality: {
        message: jobsWithCostData === 0 ? "No cost data available. Link bank transactions to invoices to see accurate margin analysis." : jobsWithCostData < invoices.length ? `${jobsWithCostData} of ${invoices.length} invoices have cost data. Margins for invoices without cost data assume 100% margin.` : "All invoices have cost data for accurate margin analysis."
      }
    }), {
      status: 200,
      headers: {
