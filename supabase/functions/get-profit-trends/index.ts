 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    const { data: invoices, error: invoicesError } = await supabaseClient.from("invoices").select(`
      ...
      `).eq("user_id", user.id).gte("invoice_date", startDate.toISOString().split('T')[0]).lte("invoice_date", endDate.toISOString().split('T')[0]).order("invoice_date", {
      ascending: true
    });
    if (invoicesError) {
      throw invoicesError;
    ...
    // ============================================
    const { data: transactions, error: txError } = await supabaseClient.from("transactions").select("amount, date").eq("user_id", user.id).gte("date", startDate.toISOString().split('T')[0]).lte("date", endDate.toISOString().split('T')[0]).order("date", {
      ascending: true
    });
    if (txError) {
      throw txError;
    ...
    const getKey = period === "quarterly" ? getQuarterKey : period === "yearly" ? getYearKey : getMonthKey;
    // Aggregate invoices
    ...
    for(let i = 1; i < trends.length; i++){
      ...
      const profitGrowth = previous.job_profit > 0 ? (current.job_profit - previous.job_profit) / previous.job_profit * 100 : 0;
      const netProfitGrowth = previous.net_profit > 0 ? (current.net_profit - previous.net_profit) / previous.net_profit * 100 : 0;
      growthRates.push({
    ...
    const recentAvgRevenue = recentPeriods.length > 0 ? recentPeriods.reduce((sum, t)=>sum + t.revenue, 0) / recentPeriods.length : 0;
    const olderAvgRevenue = olderPeriods.length > 0 ? olderPeriods.reduce((sum, t)=>sum + t.revenue, 0) / olderPeriods.length : 0;
    const trendDirection = recentAvgRevenue > olderAvgRevenue ? "growing" : recentAvgRevenue < olderAvgRevenue ? "declining" : "stable";
    ...
    return new Response(JSON.stringify({
      ...
      data_quality: {
        message: totalInvoicesWithCostData === 0 ? "No cost data available. Link bank transactions to invoices for profit trend analysis." : totalInvoicesWithCostData < totalInvoices ? `${totalInvoicesWithCostData} of ${totalInvoices} invoices have cost data. Profit trends may not be fully accurate.` : "All invoices have cost data for accurate trend analysis.",
        cost_data_coverage: totalInvoices > 0 ? parseFloat((totalInvoicesWithCostData / totalInvoices * 100).toFixed(2)) : 0
      }
    }), {
      status: 200,
      headers: {
