...
serve(async (req)=>{
  ...
  try {
    ...
    let query = supabaseClient.from("plaid_items").select("*").eq("user_id", user.id).eq("status", "active");
    if (plaid_item_id) {
      ...
      try {
        ...
        if (modified && modified.length > 0) {
          for (const txn of modified){
            const { error: updateError } = await supabaseClient.from("transactions").update({
              date: txn.date,
              amount: txn.amount,
              name: txn.name,
              merchant_name: txn.merchant_name,
        ...
        if (removed && removed.length > 0) {
          const { error: deleteError } = await supabaseClient.from("transactions").delete().in("transaction_id", removed.map((txn)=>txn.transaction_id));
          if (!deleteError) {
