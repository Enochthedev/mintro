 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    const { data: expenses, error: expenseError } = await supabaseClient.from("quickbooks_expenses").select("*").eq("user_id", user.id).not("customer_ref_id", "is", null);
    if (expenseError) throw expenseError;
    console.log(`Found ${expenses?.length || 0} expenses with customer refs`);
    // Get invoices to process
    let invoicesQuery = supabaseClient.from("invoices").select("id, quickbooks_customer_ref, client, amount, service_type, invoice_date").eq("user_id", user.id);
    if (invoice_id) {
      invoicesQuery = invoicesQuery.eq("id", invoice_id);
    } else if (!auto_link_all) {
      // Only process invoices without linked QB expenses
      invoicesQuery = invoicesQuery.is("cost_data_source", null).or("cost_data_source.eq.estimated,cost_data_source.eq.keyword_fallback");
    }
    ...
    for (const invoice of invoices || []){
      ...
      if (matchedExpenses.length > 0) {
        ...
        for (const exp of matchedExpenses){
          ...
          if (accountName.includes("material") || accountName.includes("cogs") || accountName.includes("inventory") || vendorName.includes("depot") || vendorName.includes("lowes") || vendorName.includes("supply")) {
            materialsCost += amount;
          } else if (accountName.includes("labor") || accountName.includes("subcontract") || accountName.includes("payroll") || vendorName.includes("contractor")) {
            laborCost += amount;
          } else {
            overheadCost += amount;
          }
