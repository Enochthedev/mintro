...
serve(async (req)=>{
  ...
  try {
    ...
    if (!force_new && !plaid_item_id) {
      const { data: cachedToken } = await supabaseClient.from("link_tokens").select("token, created_at").eq("user_id", user.id).is("plaid_item_id", null).order("created_at", {
        ascending: false
      }).limit(1).single();
      if (cachedToken) {
    ...
    const { data: recentRequests } = await supabaseClient.from("link_tokens").select("created_at").eq("user_id", user.id).gte("created_at", new Date(Date.now() - 60000).toISOString()).order("created_at", {
      ascending: false
    });
    ...
    if (plaid_item_id) {
      const { data: plaidItem } = await supabaseClient.from("plaid_items").select("access_token").eq("id", plaid_item_id).eq("user_id", user.id).single();
      if (!plaidItem) {
        return new Response(JSON.stringify({
    ...
    const { error: cacheError } = await supabaseClient.from("link_tokens").insert({
      user_id: user.id,
