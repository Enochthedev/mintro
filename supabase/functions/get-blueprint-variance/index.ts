...
serve(async (req)=>{
  ...
  try {
    ...
    let query = supabaseClient.from("blueprint_usage").select(`
        *,
        cost_blueprints (
          id,
          name,
    ...
    const { data: usages, error: usageError } = await query.order("completed_date", {
      ascending: false
    });
    if (usageError) throw usageError;
    // Calculate variances
    const variances = (usages || []).map((usage)=>{
      ...
      return {
        ...
        variance_percentage: {
          materials: bp.estimated_materials_cost > 0 ? materialsVariance / bp.estimated_materials_cost * 100 : 0,
          labor: bp.estimated_labor_cost > 0 ? laborVariance / bp.estimated_labor_cost * 100 : 0,
          overhead: bp.estimated_overhead_cost > 0 ? overheadVariance / bp.estimated_overhead_cost * 100 : 0,
          total_cost: bp.total_estimated_cost > 0 ? totalCostVariance / bp.total_estimated_cost * 100 : 0,
          profit: targetProfit > 0 ? profitVariance / targetProfit * 100 : 0
        },
    ...
    const avgMaterialsVariance = totalJobs > 0 ? variances.reduce((sum, v)=>sum + v.variance.materials, 0) / totalJobs : 0;
    const avgLaborVariance = totalJobs > 0 ? variances.reduce((sum, v)=>sum + v.variance.labor, 0) / totalJobs : 0;
    const avgOverheadVariance = totalJobs > 0 ? variances.reduce((sum, v)=>sum + v.variance.overhead, 0) / totalJobs : 0;
    const avgTotalVariance = totalJobs > 0 ? variances.reduce((sum, v)=>sum + v.variance.total_cost, 0) / totalJobs : 0;
