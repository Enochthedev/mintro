...
serve(async (req)=>{
  ...
  try {
    ...
    const totalIncome = formattedTransactions.filter((tx)=>parseFloat(tx.amount) < 0).reduce((sum, tx)=>sum + Math.abs(parseFloat(tx.amount)), 0);
    const totalExpenses = formattedTransactions.filter((tx)=>parseFloat(tx.amount) > 0).reduce((sum, tx)=>sum + parseFloat(tx.amount), 0);
    return new Response(JSON.stringify({
      success: true,
