...
serve(async (req)=>{
  ...
  try {
    ...
    if (invoice_id) {
      ...
      const { data: invoice, error: invoiceError } = await supabaseClient.from("invoices").select("*, invoice_items(*)").eq("id", invoice_id).eq("user_id", user.id).single();
      ...
      const { data: syncedMappings } = await supabaseClient.from("quickbooks_invoice_mappings").select("our_invoice_id").eq("quickbooks_connection_id", connection.id);
      const syncedIds = (syncedMappings || []).map((m)=>m.our_invoice_id);
      let query = supabaseClient.from("invoices").select("*, invoice_items(*)").eq("user_id", user.id);
    ...
    try {
      for (const invoice of invoicesToPush){
        try {
          ...
          const { data: existingMapping } = await supabaseClient.from("quickbooks_invoice_mappings").select("quickbooks_invoice_id").eq("quickbooks_connection_id", connection.id).eq("our_invoice_id", invoice.id).single();
          ...
          if (existingMapping) {
            ...
            const fetchResponse = await fetch(`${QUICKBOOKS_API_BASE_URL}/v3/company/${connection.realm_id}/invoice/${existingMapping.quickbooks_invoice_id}`, {
              headers: {
            ...
            qbResponse = await fetch(`${QUICKBOOKS_API_BASE_URL}/v3/company/${connection.realm_id}/invoice`, {
              method: "POST",
              headers: {
                "Authorization": `Bearer ${connection.access_token}`,
                "Accept": "application/json",
      ...
      await supabaseClient.from("quickbooks_sync_logs").insert(logEntry);
    }
  } catch (error) {
