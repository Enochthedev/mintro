...
serve(async (req)=>{
  ...
  try {
    ...
    const { rule_id, category_id, rule_type, match_value, min_amount, max_amount, priority, confidence_score, is_active } = await req.json();
    // Validation
    if (!rule_id) {
      return new Response(JSON.stringify({
    ...
    const { data: existingRule, error: fetchError } = await supabaseClient.from("categorization_rules").select("*").eq("id", rule_id).eq("user_id", user.id).single();
    ...
    if (category_id !== undefined) {
      const { data: category, error: catError } = await supabaseClient.from("expense_categories").select("id").eq("id", category_id).eq("user_id", user.id).single();
    ...
    if (match_value !== undefined) {
      if (typeof match_value !== 'string' || match_value.trim().length === 0) {
        return new Response(JSON.stringify({
          error: "match_value must be a non-empty string"
        }), {
          status: 400,
          headers: {
            ...corsHeaders,
    ...
    // Perform update
    const { data: rule, error: updateError } = await supabaseClient.from("categorization_rules").update(updates).eq("id", rule_id).eq("user_id", user.id).select(`
        *,
