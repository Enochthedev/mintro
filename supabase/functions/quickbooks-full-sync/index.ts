...
serve(async (req)=>{
  ...
  try {
    ...
    try {
      ...
      const itemResponse = await fetch(`${baseUrl}/v3/company/${qbAuth.realm_id}/query?query=${itemQuery}`, {
        headers: {
    ...
    try {
      ...
      const accountResponse = await fetch(`${baseUrl}/v3/company/${qbAuth.realm_id}/query?query=${accountQuery}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          Accept: "application/json"
        }
    ...
    try {
      ...
      const invoiceResponse = await fetch(`${baseUrl}/v3/company/${qbAuth.realm_id}/query?query=${invoiceQuery}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          Accept: "application/json"
        }
      ...
      if (invoiceResponse.ok) {
        ...
        for (const qbInvoice of qbInvoices){
          try {
            ...
            const { data: existing } = await supabaseClient.from("invoices").select("id, edited_after_sync, cost_data_source").eq("user_id", user.id).eq("quickbooks_id", qbInvoice.Id).single();
            const invoiceRecord = {
              user_id: user.id,
            ...
            if (existing) {
              const { error } = await supabaseClient.from("invoices").update(invoiceRecord).eq("id", existing.id);
              if (error) {
              ...
              const { data: newInvoice, error } = await supabaseClient.from("invoices").insert(invoiceRecord).select("id").single();
              if (error) {
    ...
    try {
      ...
      const purchaseResponse = await fetch(`${baseUrl}/v3/company/${qbAuth.realm_id}/query?query=${purchaseQuery}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          Accept: "application/json"
        }
    ...
    try {
      ...
      const billResponse = await fetch(`${baseUrl}/v3/company/${qbAuth.realm_id}/query?query=${billQuery}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          Accept: "application/json"
        }
    ...
    const profitMargin = results.cost_calculation.total_revenue > 0 ? totalProfit / results.cost_calculation.total_revenue * 100 : 0;
    return new Response(JSON.stringify({
      success: true,
      ...
      data_quality: {
        items_with_cost_percentage: results.items.synced > 0 ? parseFloat((results.items.with_cost / results.items.synced * 100).toFixed(1)) : 0,
        invoices_with_real_cost_percentage: results.invoices.synced + results.invoices.updated > 0 ? parseFloat((results.cost_calculation.invoices_with_real_costs / (results.invoices.synced + results.invoices.updated) * 100).toFixed(1)) : 0,
        recommendation: results.items.with_cost < results.items.synced * 0.5 ? "Add PurchaseCost to your QuickBooks Items for more accurate profit tracking" : "Good coverage of item costs!"
      }
    }), {
      status: 200,
      headers: {
 ...
 */ function calculateRealCosts(lines, itemCostMap) {
  let materials = 0;
  let labor = 0;
  let overhead = 0;
 ...
 */ function buildLineItemsJsonb(lines, itemCostMap) {
  return (lines || []).filter((line)=>line.DetailType === "SalesItemLineDetail").map((line)=>{
 ...
 */ async function syncLineItems(supabaseClient, invoiceId, lines, itemCostMap) {
 ...
 */ function mapQBStatus(balance, total) {
