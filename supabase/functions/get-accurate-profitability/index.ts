 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    const { data: invoices, error: invoicesError } = await supabaseClient.from("invoices").select(`
        ...
        line_items
      `).eq("user_id", user.id).gte("invoice_date", startDate).lte("invoice_date", endDate).order("invoice_date", {
      ascending: false
    });
    ...
    // ============================================
    const { data: transactions, error: txError } = await supabaseClient.from("transactions").select("id, amount, date, category, name, merchant_name").eq("user_id", user.id).gte("date", startDate).lte("date", endDate);
    ...
    invoiceDetails.forEach((inv)=>{
      ...
      const current = serviceTypeMap.get(type) || {
        revenue: 0,
        cost: 0,
        count: 0,
        realCostCount: 0
 ...
 */ function getDataQuality(costSource) {
  switch(costSource){
    case "qb_item_cost":
      return {
        level: "excellent",
        description: "Real cost from QuickBooks Item.PurchaseCost"
      };
    case "qb_expense_linked":
      return {
        level: "excellent",
        description: "Real cost from linked QuickBooks expenses"
      };
    case "user_verified":
      return {
        level: "good",
    ...
    case "transaction_linked":
      return {
        level: "good",
        description: "Cost from linked bank transactions"
      };
    case "blueprint_linked":
      return {
        level: "fair",
        description: "Estimated from cost blueprint"
      };
    case "chart_of_accounts":
      return {
        level: "fair",
        description: "Estimated from account type classification"
      };
    case "estimated":
      return {
        level: "poor",
        description: "Rough estimate based on industry averages"
      };
    case "keyword_fallback":
      return {
        level: "poor",
        description: "Estimated from item name keywords"
      };
    default:
      return {
        level: "none",
