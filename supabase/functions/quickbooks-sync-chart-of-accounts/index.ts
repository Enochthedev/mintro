  ...
  try {
    ...
    for (const account of qbAccounts){
      try {
        ...
        const { error: upsertError } = await supabaseClient.from("quickbooks_chart_of_accounts").upsert(accountData, {
    ...
    // Summary by category
    const { data: allAccounts } = await supabaseClient.from("quickbooks_chart_of_accounts").select("mintro_category, account_type, name").eq("user_id", user.id);
    const categoryBreakdown = {};
    const accountTypesFound = {};
