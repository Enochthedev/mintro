 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    // ============================================
    const { data: recentInvoices, error: invoicesError } = await supabaseClient.from("invoices").select(`
        id,
        invoice,
        client,
        amount,
      ...
      `).eq("user_id", user.id).gte("invoice_date", startDate.toISOString().split('T')[0]);
    if (invoicesError) {
      throw invoicesError;
    }
    // ============================================
    ...
    const invoiceCalculations = (recentInvoices || []).map((inv)=>{
      ...
      // Get blueprint info
      const blueprints = (inv.blueprint_usage || []).map((usage)=>({
    ...
    // ============================================
    const lowMarginJobs = invoiceCalculations.filter((job)=>job.has_cost_data && job.margin < margin_threshold && job.margin >= 0).sort((a, b)=>a.margin - b.margin).slice(0, 10).map((job)=>({
    ...
    // ============================================
    const negativeJobs = invoiceCalculations.filter((job)=>job.has_cost_data && job.profit < 0).sort((a, b)=>a.profit - b.profit).slice(0, 10).map((job)=>({
        invoice_id: job.invoice_id,
        invoice_number: job.invoice_number,
        client: job.client,
        service_type: job.service_type,
    ...
    // ============================================
    const costSpikes = invoiceCalculations.filter((job)=>job.has_cost_data && job.cost_variance_percent !== null && job.cost_variance_percent > cost_spike_threshold).sort((a, b)=>(b.cost_variance_percent || 0) - (a.cost_variance_percent || 0)).slice(0, 10).map((job)=>({
        invoice_id: job.invoice_id,
        invoice_number: job.invoice_number,
        client: job.client,
        service_type: job.service_type,
    ...
    const { data: historicalInvoices } = await supabaseClient.from("invoices").select("id, amount, invoice_date, total_actual_cost, cost_override_by_user").eq("user_id", user.id).gte("invoice_date", threeMonthsAgo.toISOString().split('T')[0]).order("invoice_date", {
      ascending: true
    });
    // Get allocations for historical invoices too
    const historicalIds = historicalInvoices?.map((inv)=>inv.id) || [];
    const { data: historicalAllocations } = await supabaseClient.from("transaction_job_allocations").select("job_id, allocation_amount").in("job_id", historicalIds.length > 0 ? historicalIds : [
      '00000000-0000-0000-0000-000000000000'
    ]);
    const historicalAllocationsByInvoice = new Map();
    ...
    const underperformingBlueprints = Array.from(blueprintPerformance.entries()).map(([id, data])=>{
    ...
    }).filter((bp)=>bp.average_margin < margin_threshold).sort((a, b)=>a.average_margin - b.average_margin).slice(0, 5);
    // ============================================
    // 6. INVOICES WITHOUT COST DATA
    // ============================================
    const invoicesWithoutCostData = invoiceCalculations.filter((inv)=>!inv.has_cost_data).slice(0, 5).map((inv)=>({
    ...
    const totalAlerts = lowMarginJobs.length + negativeJobs.length + costSpikes.length + underperformingBlueprints.length + (decliningTrend ? 1 : 0);
    ...
    return new Response(JSON.stringify({
      ...
      recommendations: generateRecommendations(lowMarginJobs.length, negativeJobs.length, costSpikes.length, decliningTrend, invoicesWithoutCostData.length, totalInvoices),
      data_quality: {
        message: invoicesWithCostData === 0 ? "No cost data available. Link bank transactions to invoices to see margin alerts." : invoicesWithCostData < totalInvoices ? `${invoicesWithCostData} of ${totalInvoices} invoices have cost data. Alerts may be incomplete.` : "All invoices have cost data for comprehensive alerting.",
        cost_data_coverage: totalInvoices > 0 ? parseFloat((invoicesWithCostData / totalInvoices * 100).toFixed(2)) : 0
      }
    }), {
      status: 200,
      headers: {
...
function generateRecommendations(lowMarginCount, negativeCount, spikeCount, decliningTrend, missingCostDataCount, totalInvoices) {
