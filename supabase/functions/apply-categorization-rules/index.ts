...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: rules, error: rulesError } = await supabaseClient.from("categorization_rules").select("*").eq("user_id", user.id).eq("is_active", true).order("priority", {
      ascending: false
    });
    if (rulesError) {
      throw rulesError;
    ...
    const { data: categorizedTransactions, error: catError } = await supabaseClient.from("transaction_categorizations").select("transaction_id");
    if (catError) {
      throw catError;
    }
    const categorizedIds = categorizedTransactions?.map((c)=>c.transaction_id) || [];
    // FIXED: Get uncategorized transactions using filter
    let query = supabaseClient.from("transactions").select("id, name, merchant_name, amount").eq("user_id", user.id).limit(100);
    ...
    for (const tx of transactions){
      ...
      for (const rule of rules){
        ...
        if (matched) {
          ...
          const { error: insertError } = await supabaseClient.from("transaction_categorizations").insert({
