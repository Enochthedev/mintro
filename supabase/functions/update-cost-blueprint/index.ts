...
serve(async (req)=>{
  ...
  try {
    ...
    const { blueprint_id, name, description, blueprint_type, estimated_materials_cost, estimated_labor_cost, estimated_overhead_cost, target_sale_price, target_profit_margin, estimated_hours, billing_frequency, is_active, inventory_items } = await req.json();
    if (!blueprint_id) {
      return new Response(JSON.stringify({
    ...
    const { data: existingBlueprint, error: checkError } = await supabaseClient.from("cost_blueprints").select("*").eq("id", blueprint_id).eq("user_id", user.id).single();
    ...
    // Update blueprint
    const { data: updatedBlueprint, error: updateError } = await supabaseClient.from("cost_blueprints").update(updates).eq("id", blueprint_id).select().single();
    if (updateError) {
      throw updateError;
    }
    ...
    if (inventory_items && Array.isArray(inventory_items)) {
      ...
      await supabaseClient.from("blueprint_inventory_items").delete().eq("blueprint_id", blueprint_id);
      // Insert new associations
      if (inventory_items.length > 0) {
        const inventoryInserts = inventory_items.map((item)=>({
            blueprint_id,
        ...
        const { error: inventoryError } = await supabaseClient.from("blueprint_inventory_items").insert(inventoryInserts);
        if (inventoryError) {
    ...
    const { data: fullBlueprint } = await supabaseClient.from("cost_blueprints").select(`
        *,
        blueprint_inventory_items (
          *,
          inventory_items (*)
        )
      `).eq("id", blueprint_id).single();
    return new Response(JSON.stringify({
      success: true,
