...
serve(async (req)=>{
  ...
  try {
    ...
    if (!blueprint_usage_id) {
      return new Response(JSON.stringify({
        error: "blueprint_usage_id query parameter is required"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: usage, error: usageError } = await supabaseClient.from("blueprint_usage").select(`
        *,
        cost_blueprints (
          id,
          name,
    ...
    const { data: allocations, error: allocError } = await supabaseClient.from("blueprint_expense_allocations").select(`
        *,
        transactions (
          id,
          transaction_id,
      ...
      `).eq("blueprint_usage_id", blueprint_usage_id).order("created_at", {
      ascending: false
    });
    if (allocError) {
      throw allocError;
