...
serve(async (req)=>{
  ...
  try {
    ...
    if (webhook_code === "SYNC_UPDATES_AVAILABLE") {
      ...
      const { data: plaidItem, error: fetchError } = await supabaseClient.from("plaid_items").select("*").eq("item_id", item_id).single();
      ...
      try {
        ...
        const { data: bankAccounts } = await supabaseClient.from("bank_accounts").select("id, account_id").eq("plaid_item_id", plaidItem.id);
