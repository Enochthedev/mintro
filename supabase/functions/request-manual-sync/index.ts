...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile } = await supabaseClient.from("business_profiles").select("subscription_tier").eq("user_id", user.id).single();
    ...
    if (profile.subscription_tier === 'premium') {
      ...
      const { count: syncCount } = await supabaseClient.from("plaid_sync_logs").select("id", {
        count: "exact",
        head: true
      }).eq("user_id", user.id).gte("started_at", `${today}T00:00:00Z`).eq("status", "success");
    ...
    const { data: plaidItems } = await supabaseClient.from("plaid_items").select("id, institution_name").eq("user_id", user.id).eq("status", "active");
