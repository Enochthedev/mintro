 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    const { data: allocations, error: allocError } = await supabaseClient.from("transaction_job_allocations").select("job_id, allocation_amount").in("job_id", invoiceIds.length > 0 ? invoiceIds : [
    ...
    (invoices || []).forEach((inv)=>{
      ...
      const estimatedCost = (inv.blueprint_usage || []).reduce((sum, usage)=>sum + parseFloat(usage.cost_blueprints?.total_estimated_cost || 0), 0);
      // Actual Cost
      let actualCost = 0;
    ...
    return new Response(JSON.stringify({
      ...
      summary: summary,
      performance_status: summary.net_cost_variance <= 0 ? "under_budget" // Good (Under budget)
       : "over_budget",
      message: summary.jobs_with_estimates === 0 ? "No jobs with estimates found in this period." : `Tracking ${Math.abs(summary.net_cost_variance).toFixed(2)} ${summary.net_cost_variance <= 0 ? 'under' : 'over'} budget across ${summary.jobs_with_estimates} estimated jobs.`
    }), {
      status: 200,
      headers: {
        ...corsHeaders,
