...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: blueprint, error: blueprintError } = await supabaseClient.from("cost_blueprints").select("*").eq("id", blueprint_id).eq("user_id", user.id).single();
    ...
    const { data: usageRecords, error: usageError } = await supabaseClient.from("blueprint_usage").select("id, invoice_id, completed_date").eq("blueprint_id", blueprint_id).limit(5);
    if (usageError) {
    ...
    if (hasUsage && !permanent) {
      const { data: deactivated, error: deactivateError } = await supabaseClient.from("cost_blueprints").update({
        is_active: false,
        updated_at: new Date().toISOString()
      }).eq("id", blueprint_id).select().single();
      if (deactivateError) {
    ...
    if (permanent) {
      ...
      if (hasUsage) {
        await supabaseClient.from("blueprint_usage").delete().eq("blueprint_id", blueprint_id);
      }
      ...
      await supabaseClient.from("blueprint_transaction_rules").delete().eq("blueprint_id", blueprint_id);
      // Delete the blueprint itself
      const { error: deleteError } = await supabaseClient.from("cost_blueprints").delete().eq("id", blueprint_id);
      if (deleteError) {
        throw deleteError;
      }
      return new Response(JSON.stringify({
