...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: plaidItem, error: itemError } = await supabaseClient.from("plaid_items").select("*").eq("id", plaid_item_id).eq("user_id", user.id).single();
    ...
    const { data: existingAccounts } = await supabaseClient.from("bank_accounts").select("account_id").eq("plaid_item_id", plaid_item_id);
    const existingAccountIds = new Set(existingAccounts?.map((acc)=>acc.account_id) || []);
    const newAccounts = accountsData.accounts.filter((acc)=>!existingAccountIds.has(acc.account_id));
    let accountsAdded = 0;
    if (newAccounts.length > 0) {
      ...
      const { error: insertError } = await supabaseClient.from("bank_accounts").insert(accountsToInsert);
      if (insertError) {
