...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: qbReport } = await supabaseClient.from("quickbooks_pnl_reports").select("*").eq("user_id", user.id).eq("start_date", start_date).eq("end_date", end_date).order("synced_at", {
      ascending: false
    }).limit(1).single();
    const basePnl = {
    ...
    const { data: mintroInvoices, error: mintroError } = await supabaseClient.from("invoices").select("amount, total_actual_cost").eq("user_id", user.id).neq("source", "quickbooks").gte("invoice_date", start_date).lte("invoice_date", end_date);
    if (mintroError) throw mintroError;
    const mintroRevenue = mintroInvoices?.reduce((sum, inv)=>sum + (Number(inv.amount) || 0), 0) || 0;
    const mintroExpenses = mintroInvoices?.reduce((sum, inv)=>sum + (Number(inv.total_actual_cost) || 0), 0) || 0;
    ...
    const { data: editedQbInvoices, error: editedError } = await supabaseClient.from("invoices").select("amount, total_actual_cost, original_qb_amount, original_qb_cost, quickbooks_invoice_id").eq("user_id", user.id).eq("source", "quickbooks").eq("edited_after_sync", true).gte("invoice_date", start_date).lte("invoice_date", end_date);
