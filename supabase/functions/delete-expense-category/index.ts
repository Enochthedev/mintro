...
serve(async (req)=>{
  ...
  try {
    ...
    if (merge_into_category_id) {
      ...
      const { data: mergeTarget, error: mergeError } = await supabaseClient.from("expense_categories").select("id, name").eq("id", merge_into_category_id).eq("user_id", user.id).single();
    ...
    const { data: existingCategory, error: fetchError } = await supabaseClient.from("expense_categories").select("*").eq("id", category_id).eq("user_id", user.id).single();
    ...
    const { count: categorizationCount } = await supabaseClient.from("transaction_categorizations").select("*", {
      count: "exact",
      head: true
    }).eq("category_id", category_id);
    const { count: ruleCount } = await supabaseClient.from("categorization_rules").select("*", {
      count: "exact",
      head: true
    }).eq("category_id", category_id);
    ...
    if (hasLinkedData) {
      if (merge_into_category_id) {
        ...
        const { data: updatedRules } = await supabaseClient.from("categorization_rules").update({
          category_id: merge_into_category_id
        }).eq("category_id", category_id).eq("user_id", user.id).select();
        mergedRules = updatedRules?.length || 0;
        const { data: updatedCategorizations } = await supabaseClient.from("transaction_categorizations").update({
          category_id: merge_into_category_id,
          previous_category_id: category_id
        }).eq("category_id", category_id).select();
        ...
        await supabaseClient.from("expense_categories").update({
          parent_category_id: merge_into_category_id
        }).eq("parent_category_id", category_id).eq("user_id", user.id);
      } else if (force) {
        // Force delete: Remove all linked data
        const { data: deletedRuleData } = await supabaseClient.from("categorization_rules").delete().eq("category_id", category_id).eq("user_id", user.id).select();
        deletedRules = deletedRuleData?.length || 0;
        const { data: deletedCategorizationData } = await supabaseClient.from("transaction_categorizations").delete().eq("category_id", category_id).select();
    ...
    // Delete the category
    const { error: deleteError } = await supabaseClient.from("expense_categories").delete().eq("id", category_id).eq("user_id", user.id);
    if (deleteError) {
      throw deleteError;
    }
    return new Response(JSON.stringify({
      success: true,
      message: merge_into_category_id ? "Category merged and deleted successfully" : "Category deleted successfully",
      deleted_category: {
