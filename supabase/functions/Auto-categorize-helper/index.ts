...
export async function autoCategorizeTransactions(supabaseClient, userId, transactionIds) {
  ...
  try {
    ...
    const { data: rules, error: rulesError } = await supabaseClient.from("categorization_rules").select("*").eq("user_id", userId).eq("is_active", true).order("priority", {
      ascending: false
    });
    if (rulesError) {
    ...
    const { data: transactions, error: txError } = await supabaseClient.from("transactions").select("id, name, merchant_name, category").eq("user_id", userId).in("id", transactionIds).is("category", null);
    if (txError) {
    ...
    for (const transaction of transactions){
      ...
      for (const rule of rules){
        ...
        const isMatch = keywords.some((keyword)=>searchText.includes(keyword.toLowerCase()));
        if (isMatch) {
          matchedRule = rule;
          break;
        }
