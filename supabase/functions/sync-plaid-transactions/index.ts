...
const PLAID_BASE_URL = PLAID_ENV === "production" ? "https://production.plaid.com" : PLAID_ENV === "development" ? "https://development.plaid.com" : "https://sandbox.plaid.com";
const corsHeaders = {
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: plaidItems, error: fetchError } = await supabaseClient.from("plaid_items").select(`
        *,
        business_profiles!inner(subscription_tier, user_id)
      `).eq("status", "active").eq("needs_sync", true).limit(50); // Process max 50 items per run
    ...
    for (const item of plaidItems){
      ...
      const transactionIdsToCategorizze = [];
      try {
        ...
        if (!syncResponse.ok) {
          ...
          if (syncData.error_code === "ITEM_LOGIN_REQUIRED") {
            await supabaseClient.from("plaid_items").update({
        ...
        const { data: bankAccounts } = await supabaseClient.from("bank_accounts").select("id, account_id").eq("plaid_item_id", item.id);
        const accountMap = new Map(bankAccounts?.map((acc)=>[
            acc.account_id,
            acc.id
          ]) || []);
        // Process added transactions
        if (added && added.length > 0) {
          const transactionsToInsert = added.map((txn)=>({
          ...
          if (transactionsToInsert.length > 0) {
            const { data: insertedData, error: insertError } = await supabaseClient.from("transactions").upsert(transactionsToInsert, {
        ...
        if (modified && modified.length > 0) {
          ...
          const { data: modifiedTxns } = await supabaseClient.from("transactions").select("id").eq("user_id", item.user_id).in("transaction_id", modified.map((t)=>t.transaction_id));
          if (modifiedTxns) {
        ...
        if (removed && removed.length > 0) {
          await supabaseClient.from("transactions").delete().in("transaction_id", removed.map((txn)=>txn.transaction_id));
        ...
        if (transactionIdsToCategorizze.length > 0) {
          ...
          try {
            const categorizationResponse = await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/auto-categorize-transactions`, {
              method: "POST",
              headers: {
