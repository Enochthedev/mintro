...
serve(async (req)=>{
  ...
  try {
    ...
    // Get transaction
    const { data: transaction, error: txError } = await supabaseClient.from("transactions").select("*").eq("id", transactionId).eq("user_id", user.id).single();
    ...
    const { data: blueprintAllocs, error: bpError } = await supabaseClient.from("blueprint_expense_allocations").select(`
        *,
        blueprint_usage (
          id,
          blueprint_id,
    ...
    const { data: jobAllocs, error: jobError } = await supabaseClient.from("transaction_job_allocations").select(`
        *,
        invoices (id, invoice, client)
      `).eq("transaction_id", transactionId);
    if (jobError) throw jobError;
    // Calculate totals
    const totalBlueprintAllocated = blueprintAllocs?.reduce((sum, alloc)=>sum + parseFloat(alloc.allocation_amount), 0) || 0;
    const totalJobAllocated = jobAllocs?.reduce((sum, alloc)=>sum + parseFloat(alloc.allocation_amount), 0) || 0;
    ...
    const allocationPercentage = transactionAmount > 0 ? totalAllocated / transactionAmount * 100 : 0;
    return new Response(JSON.stringify({
      success: true,
      transaction: {
        id: transaction.id,
