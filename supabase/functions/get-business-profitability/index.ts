 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    // ============================================
    const { data: invoices, error: invoicesError } = await supabaseClient.from("invoices").select(`
        id,
        invoice,
        client,
        amount,
    ...
    // ============================================
    let qbMergedPnl = null;
    // Check if user has QB P&L data for this period
    const { data: qbReport } = await supabaseClient.from("quickbooks_pnl_reports").select("*").eq("user_id", user.id).lte("start_date", startDate).gte("end_date", endDate).order("synced_at", {
      ascending: false
    }).limit(1).single();
    if (qbReport) {
      // Get Mintro-only invoices (not from QB)
      const { data: mintroOnlyInvoices } = await supabaseClient.from("invoices").select("amount, total_actual_cost").eq("user_id", user.id).neq("source", "quickbooks").gte("invoice_date", startDate).lte("invoice_date", endDate);
      const mintroRevenue = mintroOnlyInvoices?.reduce((sum, inv)=>sum + (Number(inv.amount) || 0), 0) || 0;
      const mintroExpenses = mintroOnlyInvoices?.reduce((sum, inv)=>sum + (Number(inv.total_actual_cost) || 0), 0) || 0;
      // Get edited QB invoices (need to apply delta adjustment)
      const { data: editedQbInvoices } = await supabaseClient.from("invoices").select("amount, total_actual_cost, original_qb_amount, original_qb_cost").eq("user_id", user.id).eq("source", "quickbooks").eq("edited_after_sync", true).gte("invoice_date", startDate).lte("invoice_date", endDate);
      let adjustmentRevenue = 0;
      let adjustmentExpenses = 0;
      editedQbInvoices?.forEach((inv)=>{
    ...
    const { data: allAllocations, error: allocError } = await supabaseClient.from("transaction_job_allocations").select("job_id, allocation_amount").in("job_id", invoiceIds.length > 0 ? invoiceIds : [
    ...
    // ============================================
    const { data: transactions, error: txError } = await supabaseClient.from("transactions").select("amount, date, category").eq("user_id", user.id).gte("date", startDate).lte("date", endDate);
    if (txError) {
      throw txError;
    }
    // EXPENSE LOGIC:
    ...
    const invoiceCalculations = (invoices || []).map((inv)=>{
      ...
      const blueprintCosts = (inv.blueprint_usage || []).reduce((sum, usage)=>sum + parseFloat(usage.cost_blueprints?.total_estimated_cost || 0), 0);
    ...
    const averageJobProfit = invoices?.length && invoices.length > 0 ? totalCalculatedProfit / invoices.length : 0;
    const averageJobMargin = invoices?.length && invoices.length > 0 ? totalCalculatedProfit / totalRevenue * 100 : 0;
    // ============================================
    ...
    const lastMonthStart = currentMonth === 0 ? `${currentYear - 1}-12-01` : `${currentYear}-${String(currentMonth).padStart(2, '0')}-01`;
    const lastMonthEnd = currentMonth === 0 ? `${currentYear - 1}-12-31` : `${currentYear}-${String(currentMonth).padStart(2, '0')}-${new Date(currentYear, currentMonth, 0).getDate()}`;
    const currentMonthCalcs = invoiceCalculations.filter((inv)=>inv.invoice_date >= currentMonthStart);
    const lastMonthCalcs = invoiceCalculations.filter((inv)=>inv.invoice_date >= lastMonthStart && inv.invoice_date <= lastMonthEnd);
    ...
    return new Response(JSON.stringify({
      ...
      data_quality: {
        message: invoicesWithRealCosts > 0 ? `${invoicesWithRealCosts} of ${invoices?.length} invoices have real cost data from QuickBooks.` : invoicesWithAnyCostData === 0 ? "No cost data available. Run quickbooks-full-sync to get real costs from QuickBooks Items." : `${invoicesWithAnyCostData} of ${invoices?.length} invoices have estimated costs. Run quickbooks-full-sync for accurate data.`,
        ...
        recommendation: invoicesWithQbItemCost === 0 ? "Run quickbooks-full-sync to sync Items with PurchaseCost for accurate profit tracking." : invoicesWithRealCosts < (invoices?.length || 0) * 0.5 ? "Add PurchaseCost to more QuickBooks Items for better accuracy." : "Good coverage of real cost data!"
      },
 ...
 */ function getQualityLevel(costSource) {
  switch(costSource){
    case "qb_item_cost":
