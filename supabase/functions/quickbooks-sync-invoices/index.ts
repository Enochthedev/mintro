...
function extractBillingAddress(billAddr) {
...
function deriveServiceType(lines) {
...
function calculateCostsFromLineItems(lines, accountMap) {
  ...
  for (const line of lines){
    ...
    if (!classified) {
      ...
      const isProduct = accountName.includes("product") || accountName.includes("materials") || accountName.includes("inventory") || accountName.includes("cost of goods") || [
        "fountain",
        "pump",
        "rocks",
        "sod",
        ...
        "heads",
        "concrete"
      ].some((p)=>itemName.includes(p));
      // Check if this is labor/service
      const isLabor = accountName.includes("service") || accountName.includes("labor") || accountName.includes("installation") || [
        "design",
        "gardening",
        "trimming",
        "installation",
  ...
  let classification_method = "none";
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: connection, error: connectionError } = await supabaseClient.from("quickbooks_connections").select("*").eq("user_id", user.id).eq("status", "active").single();
    ...
    if (force_resync) {
      ...
      const { data: qbInvoices } = await supabaseClient.from("invoices").select("id").eq("user_id", user.id).not("quickbooks_id", "is", null);
      console.log("Found invoices with quickbooks_id:", qbInvoices?.length || 0);
      // Also get all mapped invoices
      const { data: mappings } = await supabaseClient.from("quickbooks_invoice_mappings").select("our_invoice_id");
      console.log("Found mappings:", mappings?.length || 0);
      const allIds = [
        ...(qbInvoices || []).map((i)=>i.id),
        ...(mappings || []).map((m)=>m.our_invoice_id)
      ];
      const uniqueIds = [
        ...new Set(allIds)
      ];
      ...
      if (uniqueIds.length > 0) {
        // Delete items
        await supabaseClient.from("invoice_items").delete().in("invoice_id", uniqueIds);
        // Delete invoices
        await supabaseClient.from("invoices").delete().in("id", uniqueIds);
      }
      // Delete all mappings
      await supabaseClient.from("quickbooks_invoice_mappings").delete().neq("id", "00000000-0000-0000-0000-000000000000"); // Delete all
    }
    // Query QuickBooks for invoices
    const query = "SELECT * FROM Invoice MAXRESULTS 500";
    console.log("Querying QuickBooks...");
    const invoicesResponse = await fetch(`${QUICKBOOKS_API_BASE_URL}/v3/company/${connection.realm_id}/query?query=${encodeURIComponent(query)}`, {
      headers: {
    ...
    const { data: chartOfAccounts } = await supabaseClient.from("quickbooks_chart_of_accounts").select("quickbooks_account_id, mintro_category, account_type").eq("user_id", user.id);
    ...
    for (const qbInvoice of qbInvoices){
      try {
        ...
        const { data: existing } = await supabaseClient.from("invoices").select("id, edited_after_sync").eq("user_id", user.id).eq("quickbooks_id", qbInvoice.Id).single();
        if (existing) {
          if (update_existing) {
            ...
            if (shouldUpdateFinancials) {
              ...
              updateData.actual_profit = estimatedCosts.totalCost > 0 ? Math.round((qbInvoice.TotalAmt - estimatedCosts.totalCost) * 100) / 100 : null;
              updateData.cost_data_source = estimatedCosts.totalCost > 0 ? "estimated" : null;
            }
            const { error: updateError } = await supabaseClient.from("invoices").update(updateData).eq("id", existing.id);
            if (updateError) {
        ...
        // Check by mapping
        const { data: mapping } = await supabaseClient.from("quickbooks_invoice_mappings").select("our_invoice_id").eq("quickbooks_invoice_id", qbInvoice.Id).single();
        if (mapping) {
          skippedCount++;
          continue;
        }
        ...
        const qbLineItems = (qbInvoice.Line || []).filter((line)=>line.DetailType === "SalesItemLineDetail");
        const lineItemsJsonb = qbLineItems.map((line)=>({
        ...
        const invoiceData = {
          ...
          actual_profit: estimatedCosts.totalCost > 0 ? Math.round((qbInvoice.TotalAmt - estimatedCosts.totalCost) * 100) / 100 : null,
        ...
        const { data: newInvoice, error: invoiceError } = await supabaseClient.from("invoices").insert(invoiceData).select().single();
        if (invoiceError) {
        ...
        // Create line items
        const lineItems = (qbInvoice.Line || []).filter((line)=>line.DetailType === "SalesItemLineDetail");
        ...
        // Create mapping
        await supabaseClient.from("quickbooks_invoice_mappings").insert({
    ...
    return new Response(JSON.stringify({
      ...
      message: force_resync ? `Force resync complete! Created ${syncedCount} invoices from QuickBooks` : `Synced ${syncedCount} new, updated ${updatedCount} existing, skipped ${skippedCount}`
    }), {
      status: 200,
      headers: {
        ...corsHeaders,
...
async function rebuildLineItems(supabaseClient, invoiceId, lines) {
  if (!lines) return;
  ...
  const { error } = await supabaseClient.from("invoice_items").insert(itemsToInsert);
  if (error) {
