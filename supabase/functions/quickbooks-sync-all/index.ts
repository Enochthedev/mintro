 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    const { data: qbAuth, error: qbAuthError } = await supabaseClient.from("quickbooks_connections").select("access_token, refresh_token, realm_id, token_expires_at").eq("user_id", user.id).eq("status", "active").single();
    ...
    const { data: itemsWithCost } = await supabaseClient.from("quickbooks_items").select("quickbooks_item_id, name, purchase_cost, unit_price").eq("user_id", user.id).not("purchase_cost", "is", null);
    ...
    try {
      ...
      if (invoiceResponse.ok) {
        ...
        for (const qbInvoice of qbInvoices){
          // Check if exists
          const { data: existing } = await supabaseClient.from("invoices").select("id, cost_data_source").eq("user_id", user.id).eq("quickbooks_invoice_id", qbInvoice.Id).single();
    ...
    try {
      ...
      const { data: expenses } = await supabaseClient.from("quickbooks_expenses").select("id, customer_ref_id, total_amount, account_ref_name, vendor_name").eq("user_id", user.id).not("customer_ref_id", "is", null).eq("is_linked_to_invoice", false);
      // Get all invoices with QB customer refs
      const { data: invoices } = await supabaseClient.from("invoices").select("id, quickbooks_customer_ref, amount").eq("user_id", user.id).not("quickbooks_customer_ref", "is", null);
...
function calculateCostsFromLineItems(lines, itemCostMap, accountMap) {
  let materials = 0;
  let labor = 0;
  let overhead = 0;
