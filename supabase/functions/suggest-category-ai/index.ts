...
serve(async (req)=>{
  ...
  try {
    ...
    const { transaction_id, merchant_name, description, amount, batch_mode = false, transactions = [] // For batch processing
     } = await req.json();
    // Get user's categories
    const { data: categories, error: categoriesError } = await supabaseClient.from("expense_categories").select("id, name, description").eq("user_id", user.id);
    if (categoriesError) {
    ...
    // Prepare the prompt
    const categoriesText = categories.map((c)=>`- ${c.name}: ${c.description || 'No description'}`).join('\n');
    let prompt = '';
    if (batch_mode && transactions.length > 0) {
      // Batch processing
      const transactionsText = transactions.map((t, i)=>`${i + 1}. Merchant: "${t.merchant_name}", Amount: $${t.amount}`).join('\n');
...
Respond ONLY with valid JSON in this exact format:
{
  "suggestions": [
    ...
    if (batch_mode) {
      const validatedSuggestions = parsedContent.map((suggestion)=>{
      ...
      const validatedSuggestions = parsedContent.suggestions.map((suggestion)=>{
        const category = categories.find((c)=>c.name.toLowerCase() === suggestion.category_name.toLowerCase());
        return {
          ...suggestion,
          category_id: category?.id || suggestion.category_id,
