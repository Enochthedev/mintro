...
serve(async (req)=>{
  ...
  try {
    ...
    const { inventory_item_id, name, description, sku, category, unit_cost, minimum_quantity, supplier_name, supplier_contact } = await req.json();
    ...
    const { data: updated, error: updateError } = await supabaseClient.from("inventory_items").update(updates).eq("id", inventory_item_id).eq("user_id", user.id).select().single();
    if (updateError) {
      throw updateError;
    }
    if (!updated) {
