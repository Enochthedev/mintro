  ...
  try {
    ...
    switch(webhook.webhook_type){
      ...
      case "HISTORICAL_UPDATE":
        // Mark item for sync
        const { data: item } = await supabaseClient.from("plaid_items").select("id, user_id").eq("item_id", webhook.item_id).single();
        if (item) {
      ...
      case "ITEM_ERROR":
        // Handle errors
        await supabaseClient.from("plaid_items").update({
          status: webhook.error?.error_code === "ITEM_LOGIN_REQUIRED" ? "requires_update" : "error",
