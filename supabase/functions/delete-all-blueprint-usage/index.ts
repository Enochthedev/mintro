 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    // Build the query
    let query = supabaseClient.from("blueprint_usage").select("id, blueprint_id, invoice_id, actual_sale_price, actual_materials_cost, actual_labor_cost, actual_overhead_cost, completed_date").eq("user_id", user.id);
    ...
    const totalCost = usagesToDelete.reduce((sum, u)=>sum + (u.actual_materials_cost || 0) + (u.actual_labor_cost || 0) + (u.actual_overhead_cost || 0), 0);
    ...
    // Now delete
    let deleteQuery = supabaseClient.from("blueprint_usage").delete().eq("user_id", user.id);
    if (invoice_id) {
