...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: plaidItem, error: itemError } = await supabaseClient.from("plaid_items").insert({
      user_id: user.id,
      item_id: itemId,
      access_token: accessToken,
      institution_id: institutionId,
    ...
    const { data: insertedAccounts, error: accountsError } = await supabaseClient.from("bank_accounts").insert(accountsToInsert).select();
    if (accountsError) {
      throw accountsError;
    }
    // ============================================
    ...
    const { count: categoryCount } = await supabaseClient.from("expense_categories").select("*", {
      count: "exact",
      head: true
    }).eq("user_id", user.id);
    if (!categoryCount || categoryCount === 0) {
      ...
      try {
        const setupResponse = await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/setup-default-categories`, {
          method: "POST",
          headers: {
            "Authorization": req.headers.get("Authorization"),
            "Content-Type": "application/json"
    ...
    try {
      ...
      if (syncResponse.ok && syncData.added && syncData.added.length > 0) {
        ...
        const accountMap = new Map(insertedAccounts?.map((acc)=>[
            acc.account_id,
            acc.id
          ]) || []);
        const transactionsToInsert = syncData.added.map((txn)=>({
        ...
        if (transactionsToInsert.length > 0) {
          const { data: txData, error: txError } = await supabaseClient.from("transactions").insert(transactionsToInsert).select();
          if (!txError && txData) {
    ...
    return new Response(JSON.stringify({
      success: true,
      message: transactionsAdded > 0 ? `Bank connected! Synced ${transactionsAdded} transactions${categorizedCount > 0 ? `, categorized ${categorizedCount}` : ''}.` : "Bank connected successfully.",
      institution_name: institutionName,
