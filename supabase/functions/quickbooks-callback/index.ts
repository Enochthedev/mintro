...
serve(async (req)=>{
  ...
  try {
    ...
    const companyResponse = await fetch(`${QUICKBOOKS_API_BASE_URL}/v3/company/${realmId}/companyinfo/${realmId}`, {
      headers: {
    ...
    const { data: connection, error: insertError } = await supabaseClient.from("quickbooks_connections").upsert({
      user_id: userId,
      realm_id: realmId,
      access_token,
      refresh_token,
