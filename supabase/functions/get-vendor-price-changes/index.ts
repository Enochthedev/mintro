...
serve(async (req)=>{
  ...
  try {
    ...
    // ============================================
    const { data: transactions, error: txError } = await supabaseClient.from("transactions").select("*").eq("user_id", user.id).gt("amount", 0) // Only expenses (positive amounts)
    .gte("date", startDate.toISOString().split('T')[0]).order("date", {
      ascending: true
    });
    if (txError) {
      throw txError;
    ...
    // ============================================
    const qualifiedVendors = Array.from(vendorData.entries()).filter(([_, data])=>data.transaction_count >= min_transactions).map(([vendor, data])=>{
      ...
      const monthsSpan = (data.transactions[data.transactions.length - 1].date.getTime() - data.transactions[0].date.getTime()) / (1000 * 60 * 60 * 24 * 30);
    ...
    // ============================================
    const priceIncreases = qualifiedVendors.filter((v)=>v.price_change_percent > price_change_threshold).sort((a, b)=>b.price_change_percent - a.price_change_percent);
    const priceDecreases = qualifiedVendors.filter((v)=>v.price_change_percent < -price_change_threshold).sort((a, b)=>a.price_change_percent - b.price_change_percent);
    const stablePrices = qualifiedVendors.filter((v)=>Math.abs(v.price_change_percent) <= price_change_threshold);
    // ============================================
    // SPENDING TRENDS BY VENDOR
    // ============================================
    const topVendorsBySpending = qualifiedVendors.sort((a, b)=>b.total_spent - a.total_spent).slice(0, 10);
    // ============================================
    ...
    const spendingByMonth = Array.from(monthlySpending.entries()).sort((a, b)=>a[0].localeCompare(b[0])).map(([month, vendors])=>{
      const totalMonth = Array.from(vendors.values()).reduce((sum, amt)=>sum + amt, 0);
      const topVendors = Array.from(vendors.entries()).sort((a, b)=>b[1] - a[1]).slice(0, 5).map(([vendor, amount])=>({
          vendor,
    ...
    // IMPACT ANALYSIS
    // ============================================
    const totalPriceIncreaseImpact = priceIncreases.reduce((sum, v)=>sum + v.price_change_absolute * v.frequency_per_month * 12, 0);
    ...
    return new Response(JSON.stringify({
      ...
      recommendations: generateVendorRecommendations(priceIncreases, totalPriceIncreaseImpact, totalSpending)
    }), {
      status: 200,
      headers: {
        ...corsHeaders,
...
function generateVendorRecommendations(priceIncreases, totalImpact, totalSpending) {
  ...
  recommendations.push(`⚠️ ${topIncrease.vendor} has increased prices by ${topIncrease.price_change_percent.toFixed(1)}%. Consider negotiating or finding alternatives.`);
  if (priceIncreases.length > 3) {
    recommendations.push(`📊 ${priceIncreases.length} vendors have raised prices significantly. Time to review vendor contracts.`);
  }
  const impactPercent = totalImpact / totalSpending * 100;
  if (impactPercent > 5) {
    recommendations.push(`💰 Price increases could cost you $${totalImpact.toFixed(2)} annually (${impactPercent.toFixed(1)}% of spending). Update your pricing to maintain margins.`);
  }
  const highImpactVendors = priceIncreases.filter((v)=>v.price_change_absolute * v.frequency_per_month * 12 > totalSpending * 0.01);
  if (highImpactVendors.length > 0) {
    recommendations.push(`🎯 Focus on negotiating with: ${highImpactVendors.slice(0, 3).map((v)=>v.vendor).join(", ")}`);
  }
  return recommendations;
}
