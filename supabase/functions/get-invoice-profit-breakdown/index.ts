 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    const { data: linkedExpenses, error: linkError } = await supabaseClient.from("transaction_job_allocations").select(`
        *,
        transactions (
          id,
          transaction_id,
    ...
    const transactionCosts = (linkedExpenses || []).reduce((sum, exp)=>sum + Math.abs(parseFloat(exp.allocation_amount || 0)), 0);
    ...
    const estimatedCosts = hasBlueprints ? {
      materials: invoice.blueprint_usage.reduce((sum, usage)=>sum + parseFloat(usage.cost_blueprints?.estimated_materials_cost || 0), 0),
      labor: invoice.blueprint_usage.reduce((sum, usage)=>sum + parseFloat(usage.cost_blueprints?.estimated_labor_cost || 0), 0),
      overhead: invoice.blueprint_usage.reduce((sum, usage)=>sum + parseFloat(usage.cost_blueprints?.estimated_overhead_cost || 0), 0),
      total: invoice.blueprint_usage.reduce((sum, usage)=>sum + parseFloat(usage.cost_blueprints?.total_estimated_cost || 0), 0)
    } : null;
    ...
    const { data: overrideHistory } = await supabaseClient.from("invoice_cost_overrides").select("*").eq("invoice_id", invoice_id).order("created_at", {
      ascending: false
    });
    return new Response(JSON.stringify({
      success: true,
      ...
      blueprints: invoice.blueprint_usage?.map((usage)=>({
      ...
      data_quality: {
        message: transactionCosts === 0 && !hasBlueprints && !hasOverride ? "No cost data. Link bank transactions to this invoice to track expenses." : transactionCosts === 0 && hasBlueprints ? "Using estimated costs from blueprints. Link actual transactions for accurate profit." : "Costs calculated from linked transactions."
      }
    }), {
      status: 200,
      headers: {
