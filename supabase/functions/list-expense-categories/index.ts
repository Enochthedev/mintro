...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: categories, error: categoriesError } = await supabaseClient.from("expense_categories").select("*").eq("user_id", user.id).order("name", {
      ascending: true
    });
    if (categoriesError) {
    ...
    if (includeStats && categories && categories.length > 0) {
      ...
      const { data: categorizationCounts, error: countError } = await supabaseClient.from("transaction_categorizations").select("category_id").in("category_id", categoryIds);
      if (!countError && categorizationCounts) {
        ...
        // Get rule counts
        const { data: ruleCounts, error: ruleError } = await supabaseClient.from("categorization_rules").select("category_id").in("category_id", categoryIds);
