...
serve(async (req)=>{
  ...
  try {
    ...
    // Get linked expenses
    const { data: linkedExpenses } = await supabaseClient.from("transaction_job_allocations").select(`
        *,
        transactions (
          amount,
          name,
    ...
    const context = {
      ...
      blueprint_estimate: invoice.blueprint_usage && invoice.blueprint_usage.length > 0 ? {
        materials: invoice.blueprint_usage.reduce((sum, u)=>sum + parseFloat(u.cost_blueprints?.estimated_materials_cost || 0), 0),
        labor: invoice.blueprint_usage.reduce((sum, u)=>sum + parseFloat(u.cost_blueprints?.estimated_labor_cost || 0), 0),
        overhead: invoice.blueprint_usage.reduce((sum, u)=>sum + parseFloat(u.cost_blueprints?.estimated_overhead_cost || 0), 0)
      } : null
    };
    // Call OpenAI
...
Linked Expenses:
${context.linked_expenses.length > 0 ? context.linked_expenses.map((e)=>`- ${e.vendor}: $${e.amount} (${e.category || 'uncategorized'})`).join('\n') : '- No expenses linked yet'}
