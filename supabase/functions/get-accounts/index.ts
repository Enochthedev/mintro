...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: plaidItems, error: itemsError } = await supabaseClient.from("plaid_items").select("*").eq("user_id", user.id).order("created_at", {
      ascending: false
    });
    if (itemsError) {
      throw itemsError;
    }
    const { data: accounts, error: accountsError } = await supabaseClient.from("bank_accounts").select(`
        *,
        plaid_items!inner (
          id,
          institution_name,
