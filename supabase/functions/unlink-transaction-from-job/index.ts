...
serve(async (req)=>{
  ...
  try {
    ...
    const { allocation_id, transaction_id, job_id } = await req.json();
    ...
    let query = supabaseClient.from("transaction_job_allocations").select("*, invoices(id, amount)").eq("user_id", user.id);
    if (allocation_id) {
    ...
    if (fetchError || !allocation) {
      return new Response(JSON.stringify({
        error: "Transaction allocation not found"
      }), {
        status: 404,
        headers: {
          ...corsHeaders,
    ...
    const { error: deleteError } = await supabaseClient.from("transaction_job_allocations").delete().eq("id", allocation.id);
    if (deleteError) {
      throw deleteError;
    }
    // Recalculate invoice totals after unlinking
    const { data: remainingAllocations } = await supabaseClient.from("transaction_job_allocations").select("allocation_amount").eq("job_id", invoiceId);
    const totalActualCost = remainingAllocations?.reduce((sum, alloc)=>sum + Math.abs(Number(alloc.allocation_amount) || 0), 0) || 0;
    const actualProfit = totalActualCost > 0 ? (Number(invoiceAmount) || 0) - totalActualCost : null;
